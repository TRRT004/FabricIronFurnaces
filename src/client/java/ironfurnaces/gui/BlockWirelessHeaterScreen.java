...
public class BlockWirelessHeaterScreen extends AbstractContainerScreen<BlockWirelessHeaterScreenHandler> {
	...
	public static Identifier GUI = net.minecraft.resources.Identifier.fromNamespaceAndPath(Reference.MOD_ID,
			"textures/gui/heater.png");
	...
	public BlockWirelessHeaterScreen(BlockWirelessHeaterScreenHandler handler,
	...
	protected void extractLabels(net.minecraft.client.gui.GuiGraphicsExtractor extractor, int mouseX, int mouseY) {
		...
		if (actualMouseX >= 65 && actualMouseX <= 111 && actualMouseY >= 64 && actualMouseY <= 76) {
			...
			extractor.setTooltipForNextFrame(this.font, Component
					.literal(StringHelper.displayEnergy(energy, capacity).get(0)).withStyle(ChatFormatting.GOLD),
	...
	public void extractContents(net.minecraft.client.gui.GuiGraphicsExtractor extractor, int mouseX, int mouseY,
		...
		this.blit(extractor, BlockWirelessHeaterScreen.GUI, i, j, this.imageWidth, this.imageHeight, 0.0F, 0.0F,
				(float) this.imageWidth, (float) this.imageHeight);
		int k;
		k = this.getEnergyScaled(46);
		this.blit(extractor, BlockWirelessHeaterScreen.GUI, i + 65, j + 64, k + 1, 12, 176.0F, 0.0F, (float) (k + 1),
				12.0F);
	}
	...
	private static Optional<Double> getEnergy(AbstractContainerMenu handler) {
		if (handler instanceof BlockWirelessHeaterScreenHandler) {
			...
			return pos != null
					? Optional.of(((BlockWirelessHeaterTile) world.getBlockEntity(pos)).getEnergy())
					: Optional.empty();
		} else {
	...
	private static Optional<Integer> getCapacity(AbstractContainerMenu handler) {
		if (handler instanceof BlockWirelessHeaterScreenHandler) {
			...
			Level world = ((BlockWirelessHeaterScreenHandler) handler).getLevel();
			return pos != null
					? Optional.of(((BlockWirelessHeaterTile) world.getBlockEntity(pos)).getCapacity())
					: Optional.empty();
		} else {
			return Optional.empty();
		}
